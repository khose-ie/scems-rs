...
impl<T> Drop for MemBlock<T>
{
    fn drop(&mut self)
    {
        if let Some(x) = self.mem_space.cast_opt()
        {
            unsafe { drop_in_place(x) };
            unsafe { MemPool::mfree(size_of::<T>() as u32, x.cast()) };
        }
    }
}
...
impl<T> IMemBlock<T> for MemBlock<T>
{
    ...
    fn clean(&mut self)
    {
        ...
        {
            unsafe { drop_in_place(x) };
            unsafe { write_bytes(x, 0, size_of::<T>()) };
        }
    }
}
...
impl<T> Drop for MemBlockHeap<T>
{
    fn drop(&mut self)
    {
        ...
        {
            unsafe { drop_in_place(x) };
            unsafe { MemHeap::free(x as *mut c_void) };
        }
    }
}
...
impl<const N: usize> IMemCache<N> for MemCache<N>
{
    fn set(&mut self, value: &[u8])
    {
        if let Some(x) = self.mem_space.cast_opt()
        {
            unsafe { x.copy_from(value.as_ptr(), value.len()) };
        }
    }
    ...
    fn fill(&mut self, value: u8, size: usize)
    {
        if let Some(x) = self.mem_space.cast_opt()
        {
            unsafe { write_bytes(x, value, size) };
        }
    }
    ...
    fn clean(&mut self)
    {
        if let Some(x) = self.mem_space.cast_opt()
        {
            unsafe { write_bytes(x, 0, self.size) };
        }
    }
}
